...
int  webSocketRespond::pack_data(const unsigned char* message,size_t msg_len, uint8_t fin ,
                                  uint8_t op_code, uint8_t mask, unsigned char** out, size_t* out_len)
{
    if(message == nullptr || msg_len<0 || out_len== nullptr ){
        DEBUG_LOG("pack_data parameters error!");
        return -1;
    }

    int head_length=0;                                  //记录消息头部的长度
    size_t          out_len_tmp = msg_len;              //临时变量
    unsigned char*  out_tmp     = nullptr;

    // 基本一个包可以发送完所有数据
    if(msg_len < 126)                                   //如果不需要扩展长度位, 两个字节存放 fin(1bit) + rsv[3](1bit) + op_code(4bit);
    ...
    *out_tmp = (unsigned char)(*out_tmp | (0xF & op_code));               //处理op_code, out_tmp 在 第一个字节开始处

    *(out_tmp + 1) = mask << 7;                         //开始处理第二个字节的数据, out_tmp在第一个字节的起始位置, 设置mask为 1
    if (msg_len < 126)
    {
        *(out_tmp + 1) = (unsigned char)(*(out_tmp + 1) | msg_len);      //为第二个字节的payload len赋值
        head_length += 2;                               //head_length标志着已经处理了2个字节的数据
    }
    else if (msg_len < 0xFFFF)
    {
        *(out_tmp + 1) = (unsigned char)(*(out_tmp + 1) | 0x7E);         //设置第二个字节后7bit为126
    ...
    {
        *(out_tmp + 1) = (unsigned char)(*(out_tmp + 1) | 0x7F);         //设置第二个字节后为7bit 127
        uint16_t tmp = (unsigned char)(htonl((uint16_t)msg_len));        //转换成网络字节序
        memcpy(out_tmp + 2, &tmp, sizeof(uint16_t));    //给extended payload length 赋值
